...
public class BPTSpeedTest {

    public static void main(String[] args) {
        // Print header for console table
        System.out.println("-------------------------------------------------------------");
        System.out.println("                          BPT Speed Test                     ");
        System.out.println("-------------------------------------------------------------");
        ...
        try (PrintWriter writer = new PrintWriter(Constants.BPT_CSV_PATH)) {
            // Write the header row to CSV
            writer.println("Size,Insertion Time (ms),Search Time (ms),Height");

            for (int size = Constants.START_NUM_ENTRIES; size <= Constants.END_NUM_ENTRIES; size += Constants.INCREMENT_NUM_ENTRIES) {
                // Reset the tree for each iteration
                BPTBasic bPlusTree = new BPTBasic();
                // Auto-generate numbers
                List<Integer> randomNumbers = NumberGenerator.generateRandomNumbers(size);
